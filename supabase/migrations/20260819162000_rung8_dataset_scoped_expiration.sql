...
begin
  ...
  if p_allow_expire then
    ...
    join (
      select distinct source->>'source_dataset' as source_dataset
      from _nycif_rung8_input
    ) scope on scope.source_dataset=s.source_dataset
    where s.source_name=p_source_name and s.source_active
      and not exists (
        select 1
        from _nycif_rung8_input i
        where i.occurrence_id=s.occurrence_id
          and i.source->>'source_dataset'=s.source_dataset
          and i.source->>'source_event_id'=s.source_event_id
      );

    update public.event_sources s set source_active=false,source_last_seen=now(),updated_at=now()
      from _nycif_rung8_expire x
      where s.occurrence_id=x.occurrence_id
        and s.source_name=p_source_name
        and s.source_dataset=x.source_dataset
        and s.source_event_id=x.source_event_id;
    ...
    update public.event_occurrences o set status='expired',source_active=false,last_pipeline_run_id=v_run_id,
      last_seen=now(),updated_at=now()
      from (select distinct occurrence_id from _nycif_rung8_expire) x
      where o.occurrence_id=x.occurrence_id
      and not exists (
        select 1 from public.event_sources s
        where s.occurrence_id=o.occurrence_id and s.source_active
      );
    get diagnostics v_expired = row_count;

    insert into public.event_change_log(occurrence_id,pipeline_run_id,change_type,changed_fields,reason)
      select distinct x.occurrence_id,v_run_id,'EXPIRE','["status","source_active"]'::jsonb,'RUNG8_SOURCE_ABSENT'
