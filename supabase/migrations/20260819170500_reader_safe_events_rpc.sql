create index if not exists event_occurrences_reader_window_idx
  on public.event_occurrences(start_at, end_at)
  where source_active and status = 'active';

create index if not exists event_sources_reader_active_occurrence_idx
  on public.event_sources(occurrence_id, event_source_id)
  include (source_dataset, source_event_id)
  where source_active;
