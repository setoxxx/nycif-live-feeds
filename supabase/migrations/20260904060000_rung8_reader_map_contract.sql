-- Preserve first-seen, merge reader display fields, and fail closed on
-- map_ready without coordinates. Catch-up classification does not replace
-- an existing current taxonomy row.
...
begin
  ...
  if exists (
    select 1 from _nycif_rung8_input
    where (map_ready and (lat is null or lng is null))
       or (not map_ready and (lat is not null or lng is not null))
       or nullif(metadata->'reader'->>'source_dataset','') is null
       or nullif(metadata->'reader'->>'source_event_id','') is null
       or metadata->'reader'->>'source_dataset' is distinct from source->>'source_dataset'
  ) then
    raise exception 'RUNG8_MAP_DISPLAY_CONTRACT';
  end if;
  ...
  update _nycif_rung8_input i set
    ...
    is_update = (o.occurrence_id is not null and (
      row(o.title,o.start_at,o.end_at,o.timezone,o.borough,o.display_location,o.lat,o.lng,
          o.public_category,o.public_subtype,o.status,o.source_active,o.map_ready,o.editorial_priority)
      is distinct from
      row(i.title,i.start_at,i.end_at,i.timezone,i.borough,i.display_location,i.lat,i.lng,
          i.public_category,i.public_subtype,i.status,i.source_active,i.map_ready,i.editorial_priority)
      or coalesce(o.metadata->'reader','{}'::jsonb) is distinct from coalesce(i.metadata->'reader','{}'::jsonb)
      or not exists (
        select 1 from public.event_sources s where s.occurrence_id=i.occurrence_id
          ...
          and row(s.source_cemsid,s.source_event_type,s.source_agency,s.source_url,
    ...
    classification_changed = (
      i.classification is not null
      and (
        o.occurrence_id is null
        or (
          coalesce(i.classification->>'classifier_version','') = 'official-snapshot-catchup-v1'
          and not exists (
            select 1 from public.event_classifications c
            where c.occurrence_id=i.occurrence_id and c.is_current
          )
        )
        or (
          coalesce(i.classification->>'classifier_version','') <> 'official-snapshot-catchup-v1'
  ...
  update public.event_occurrences o set
    ...
    editorial_priority=case
      when o.editorial_priority in ('high','urgent') then o.editorial_priority
      else i.editorial_priority
    end,
    last_pipeline_run_id=v_run_id,
    metadata=jsonb_set(
      coalesce(o.metadata,'{}'::jsonb) || coalesce(i.metadata,'{}'::jsonb),
      '{reader}',
      coalesce(o.metadata->'reader','{}'::jsonb)
        || coalesce(i.metadata->'reader','{}'::jsonb)
        || jsonb_build_object(
          'is_major', (
            coalesce((o.metadata->'reader'->>'is_major')::boolean,false)
            or coalesce((i.metadata->'reader'->>'is_major')::boolean,false)
          ),
          'photo_pick', (
            coalesce((o.metadata->'reader'->>'photo_pick')::boolean,false)
            or coalesce((i.metadata->'reader'->>'photo_pick')::boolean,false)
          )
        )
    ),
  ...
  on conflict (source_name,source_dataset,source_event_id,occurrence_id) do update set
    ...
    source_first_seen=coalesce(event_sources.source_first_seen, excluded.source_first_seen),
  ...
  where row(event_sources.source_cemsid,event_sources.source_event_type,event_sources.source_agency,
            event_sources.source_url,event_sources.source_last_seen,
        ...
        row(excluded.source_cemsid,excluded.source_event_type,excluded.source_agency,
            excluded.source_url,excluded.source_last_seen,
