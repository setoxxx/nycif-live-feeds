-- Certification hardening: the dataset finalizer may be invoked more than once
-- inside a reused PostgreSQL session/transaction (for example, certification
-- or retry orchestration). ON COMMIT DROP does not remove the temp table until
-- commit, so explicitly remove only the session temp table before recreating it.
