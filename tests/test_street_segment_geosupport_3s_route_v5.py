...
class Route3STests(unittest.TestCase):
    def run_audit(self, backend, v2=None, v4=None):
        return audit(copy.deepcopy(v2 or V2), copy.deepcopy(v4 or V4), backend)

    def test_ordered_route_certifies_from_v2_endpoints(self):
        result = self.run_audit(Fake3S())
        self.assertEqual(result["v2_identity_handoff_missing_count"], 0)
        self.assertEqual(result["v2_identity_handoff_not_unique_count"], 0)
        ...
        self.assertEqual(route["endpoint_1"]["node"], "0010001")
    ...
    def test_declared_count_mismatch_blocks(self):
        result = self.run_audit(Fake3S(declared=9))
        self.assertEqual(result["routes"][0]["reason_code"], "GEOSUPPORT_3S_COUNT_MISMATCH")

    def test_3s_failure_blocks(self):
        result = self.run_audit(Fake3S(fail=True))
        self.assertEqual(result["routes"][0]["reason_code"], "GEOSUPPORT_3S_UNRESOLVED")

    def test_missing_v2_handoff_blocks_without_call(self):
        v2 = copy.deepcopy(V2)
        v2["claims"] = []
        backend = Fake3S()
        result = self.run_audit(backend, v2=v2)
        self.assertEqual(result["v2_identity_handoff_missing_count"], 1)
        self.assertEqual(result["geosupport_3s_call_count"], 0)
        self.assertEqual(result["routes"][0]["reason_code"], "V2_IDENTITY_HANDOFF_MISSING")

    def test_duplicate_v2_handoff_blocks_without_call(self):
        v2 = copy.deepcopy(V2)
        v2["claims"].append(copy.deepcopy(V2_CLAIM))
        result = self.run_audit(Fake3S(), v2=v2)
        self.assertEqual(result["v2_identity_handoff_not_unique_count"], 1)
        self.assertEqual(result["geosupport_3s_call_count"], 0)
