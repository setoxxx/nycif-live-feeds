(() => {
  const VERSION = 'discovery-taxonomy-v02';
  ...
  const pageUrl = (layer, cursor) => {
    ...
    return `${FEED_HOST}/data/schema-v1-discovery/${layer}/pages/${name}.json`;
  };
  const FEEDS = {
    major: FEED_HOST + '/data/schema-v1-discovery/major/events.json',
    approvedManifest: FEED_HOST + '/data/schema-v1-discovery/approved/manifest.json',
    reviewManifest: FEED_HOST + '/data/schema-v1-discovery/review/manifest.json',
  ...
  const MAIN_CATEGORY_KEYS = ['sports', 'civic', 'market', 'arts', 'parks', 'fitness', 'family', 'education', 'volunteer', 'general'];
  const EXPLORE_CATEGORY_KEYS = ['tours', 'government', 'services', 'jobs', 'housing', 'environment'];
  const ALL_CATEGORY_KEYS = [...MAIN_CATEGORY_KEYS, ...EXPLORE_CATEGORY_KEYS];
  ...
  const els = {
    ...
    exploreMoreToggle: document.getElementById('exploreMoreToggle'),
    exploreMorePanel: document.getElementById('exploreMorePanel'),
    showAllEventsBtn: document.getElementById('showAllEventsBtn'),
    enableAllCategoriesBtn: document.getElementById('enableAllCategoriesBtn'),
  ...
  function toUiEvent(schemaEvent) {
    ...
    const interests = SCHEMA.normalizeInterests
      ? SCHEMA.normalizeInterests(catKey, schemaEvent.interests)
      : (Array.isArray(schemaEvent.interests) && schemaEvent.interests.length
        ? schemaEvent.interests
        : [catKey]);
    const eventRole = schemaEvent.event_role || 'public_event';
    const displayDisposition = nycif.display_disposition || 'standalone_public_event';
    const parentEventId = schemaEvent.parent_event_id || null;
    const coordReady = nycif.coordinate_status === 'map_ready'
      && Number.isFinite(schemaEvent.latitude)
      && Number.isFinite(schemaEvent.longitude);
    const markerEligible = coordReady
      && eventRole === 'public_event'
      && !parentEventId
      && displayDisposition === 'standalone_public_event';
    const mapReady = markerEligible;
    const listOnly = displayDisposition === 'list_only' || nycif.coordinate_status === 'list_only';
    ...
    const e = {
      ...
      relatedRecordCount: Number(nycif.related_record_count || 0),
      searchText: norm([title, location, borough, catKey, ...interests, ...(schemaEvent.tags || []), schemaEvent.source?.dataset, schemaEvent.source?.source_event_id, nycif.event_type, nycif.major_reason].filter(Boolean).join(' ')),
      marker: null
    };
  ...
  function categoryMatches(e) {
    const selected = new Set(ALL_CATEGORY_KEYS.filter(k => state.categories[k]));
    if (!selected.size) {
      return false;
    }
    return selected.has(e.categoryKey) || (e.interests || []).some(i => selected.has(i));
  }

  function majorFilterMatches(e) {
    if (!state.majorOnly) {
      return true;
    }
    return e.isMajor || e.significance === 'major';
  }
  ...
  function countForCategory(cat) {
    return state.events.filter(e => {
      if (!sourceMatches(e) || !dateMatches(e) || !majorFilterMatches(e)) {
        return false;
      }
      if (state.photoOnly && !e.photoPick) {
        return false;
      }
      if (state.nypdOnly && e.verification_status !== 'nypd_field_intel') {
        return false;
      }
      if (state.borough !== 'all' && e.borough !== state.borough) {
        return false;
      }
      if (state.search && !e.searchText.includes(state.search)) {
        return false;
      }
      return e.categoryKey === cat || (e.interests || []).includes(cat);
    }).length;
  }
  ...
  function popupRoot(e) {
    ...
    if (e.relatedRecordCount > 0) {
      addRow('Related', `${e.relatedRecordCount.toLocaleString()} supporting record${e.relatedRecordCount === 1 ? '' : 's'}`);
    }
    root.appendChild(dl);
    if (e.mapReady) {
      ...
      appendText(root, 'div', 'Not shown on map — grouped or non-public event', 'popup-photo');
    }
    return root;
  }
  ...
  function buildListCard(e) {
    ...
    if (e.isReview) {
      appendText(tags, 'span', 'REVIEW', reviewTagClass(true));
    } else {
      appendText(tags, 'span', 'LIVE', reviewTagClass(false));
    }
    if (e.listOnly) {
  ...
  function updateFilterCounts() {
    document.querySelectorAll('[data-cat-label]').forEach(span => {
      const cat = span.dataset.catLabel;
      if (!cat || !CATEGORY_META[cat]) {
        return;
      }
      const meta = CATEGORY_META[cat];
      const count = countForCategory(cat);
      span.textContent = `${meta.emoji} ${meta.label} (${count.toLocaleString()})`;
    });
  }
  ...
  function render() {
    ...
    const listOnly = visible.filter(e => e.listOnly).length;
    let indexNote = state.indexComplete ? 'full index loaded' : 'indexing more events';
    ...
    if (!visible.length) {
      ...
      appendText(empty, 'p', 'No events match this view.');
      const actions = document.createElement('div');
      actions.className = 'empty-actions';
      const resetBtn = document.createElement('button');
      resetBtn.type = 'button';
      resetBtn.className = 'load-all';
      resetBtn.textContent = 'Reset Filters';
      resetBtn.addEventListener('click', () => els.resetFiltersBtn?.click());
      const showAllBtn = document.createElement('button');
      showAllBtn.type = 'button';
      showAllBtn.className = 'load-all';
      showAllBtn.textContent = 'Show All Events';
      showAllBtn.addEventListener('click', () => els.showAllEventsBtn?.click());
      const enableBtn = document.createElement('button');
      enableBtn.type = 'button';
      enableBtn.className = 'load-all';
      enableBtn.textContent = 'Enable All Categories';
      enableBtn.addEventListener('click', () => els.enableAllCategoriesBtn?.click());
      actions.append(resetBtn, showAllBtn, enableBtn);
  ...
  function setExploreMore(open) {
    if (!els.exploreMorePanel || !els.exploreMoreToggle) {
      return;
    }
    els.exploreMorePanel.hidden = !open;
    els.exploreMoreToggle.setAttribute('aria-expanded', String(open));
  }

  function initExploreMore() {
    const mobile = window.matchMedia('(max-width: 700px)').matches;
    setExploreMore(!mobile);
  }
  ...
  async function bootFeeds() {
    ...
    try {
      const majorJson = await fetchJson(FEEDS.major, 'major');
      if (token !== state.loadToken) {
        return;
      }
  ...
  function onToggleFilterChange(input) {
    state.userChangedFilters = true;
    if (input.id === 'majorOnly') {
      state.majorOnly = input.checked;
    } else {
  ...
  function bindUi() {
    ...
    [els.majorOnly, els.photoOnly, els.nypdOnly].filter(Boolean).forEach(input => {
    ...
    els.showAllEventsBtn?.addEventListener('click', () => {
      state.userChangedFilters = true;
      state.viewMode = 'all';
    ...
    els.enableAllCategoriesBtn?.addEventListener('click', () => {
      state.userChangedFilters = true;
    ...
    els.exploreMoreToggle?.addEventListener('click', () => {
      setExploreMore(els.exploreMorePanel.hidden);
    });
    els.exploreMoreToggle?.addEventListener('keydown', ev => {
