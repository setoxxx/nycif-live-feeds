...
def v3_build_base_event(row: dict[str, Any], **kwargs: Any) -> dict[str, Any] | None:
    ...
    state = decision["map_eligibility_state"]
    exact = state == "MAP_READY" and decision["certified_pin"] is True
    ...
    if not exact:
        ...
        if nycif.get("display_disposition") not in {"maintenance_or_closure", "private_or_reserved_activity"}:
    ...
    nycif["pin_integrity_reason"] = decision.get("reason_code")
