...
def certified_row(e: dict[str, Any], *, recurrence_label: str | None = None) -> dict[str, Any] | None:
    if e.get("coordinate_status") != "map_ready":
        return None
    lat_f, lng_f, ok, reason = certify_nyc_pin(e.get("latitude"), e.get("longitude"), allow_swap_correct=True)
    ...
    for e in money_events:
        ...
        if row:
            if viral:
                ...
                row["prior_year_title"] = (viral.get("prior_year") or {}).get("title") if isinstance(viral.get("prior_year"), dict) else viral.get("prior_year_title")
                row["prior_year_date"] = (viral.get("prior_year") or {}).get("date") if isinstance(viral.get("prior_year"), dict) else viral.get("prior_year_date")
            certified.append(row)
        else:
            needs_location.append(
                {
                    "id": e.get("id"),
                    "title": e.get("title"),
                    "date": e.get("date"),
...
def main() -> int:
    ...
    viral_by_id: dict[str, dict[str, Any]] = {}
    for m in (viral_matches.get("matches") if isinstance(viral_matches, dict) else None) or []:
        cur = m.get("current") if isinstance(m, dict) else None
    ...
    # Optional next 7 summary (certified counts only)
    next7 = []
    for i in range(7):
        d = reference + timedelta(days=i)
        sec = build_day_section(d, events, viral_by_id)
        next7.append(
            {
                "date": sec["date"],
    ...
    report = {
        ...
        "qa_pass": bool(gate.get("qa_pass")) and today_sec["certified_pin_count"] >= 0,
        "reference_today_nyc": reference.isoformat(),
        ...
        "pin_integrity_qa_pass": bool(gate.get("qa_pass")),
        "today_top": [
            {"title": e.get("title"), "borough": e.get("borough"), "recurrence_label": e.get("recurrence_label")}
            for e in (today_sec.get("go_shoot_certified") or [])[:8]
        ],
        "tomorrow_top": [
            {"title": e.get("title"), "borough": e.get("borough"), "recurrence_label": e.get("recurrence_label")}
            for e in (tom_sec.get("go_shoot_certified") or [])[:8]
        ],
        "promotion_allowed": False,
        "public_map_modified": False,
        "location_cache_modified": False,
