...
def main():
    ...
    for item in remaining:
        ...
        if not result:
            ...
            if result: resolver=str(result.get('facility_resolver') or 'facility')
        if not result:
        ...
        if result.get('coordinate_precision')=='certified_facility' and normalize_name(result.get('facility_query_name'))!=normalize_name(result.get('facility_name')):
            if normalize_name(result.get('facility_query_name')) not in {normalize_name(result.get('facility_name')),normalize_name(str(result.get('facility_name'))+' library'),normalize_name(str(result.get('facility_name'))+' branch')}:
                warnings.append('query_name_not_exact_alias')
    ...
    OUT_JSON.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n'); lines=['# SHADOW-2 Authoritative Facility Delta','',f'- Baseline unresolved: **170**',f'- Resolved total: **{len(resolved)}**',f'- Remain unresolved: **{len(unresolved)}**',f'- Structural false-match warnings: **{len(mismatch)}**','', '## Resolver delta']+[f'- {k}: {v}' for k,v in sorted(counts.items())]+['','## True remainder']+[f'- {k}: {v}' for k,v in sorted(remainder_counts.items())]; OUT_MD.write_text('\n'.join(lines)+'\n')
    print(json.dumps({k:v for k,v in report.items() if k not in {'resolved_records','remaining_records'}},indent=2)); return 0 if not mismatch and report['safety']['all_approximate'] and not report['safety']['promotion_allowed_any'] else 1
