...
CLI file arguments are treated as untrusted input.  Every path is resolved
against the current workspace and must remain inside that workspace after
...
def resolve_workspace_file(raw: str, *, must_exist: bool) -> Path:
    ...
    Absolute paths and paths that resolve outside the current working directory
    are rejected.  Existing inputs must be regular files.  Output parents must
    already exist and the output path may not name a directory.
    """
